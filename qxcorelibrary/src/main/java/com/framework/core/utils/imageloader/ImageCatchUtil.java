...
public class ImageCatchUtil {
    ...
    public final String ImageExternalCatchDir = AppSetting.getContext().getCacheDir() + "/image_cache";
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(AppSetting.getContext()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(AppSetting.getContext()).clearDiskCache();
    ...
    public void clearImageMemoryCache() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(AppSetting.getContext()).clearMemory();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
